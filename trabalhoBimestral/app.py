...
@app.route('/livros')
def livros():
    command = 'SELECT * FROM Livros ORDER BY nome_livro ASC;'
...
@app.route('/usuarios')
def usuarios():
    command = 'SELECT * FROM Usuarios ORDER BY nome_usuario ASC;'
    cursor.execute(command)
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not emprestimos:
                ...
                return redirect(url_for('meusEmprestimos'))
...
def AdmEmprestimos():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not emprestimos:
                flash('Você ainda não fez nenhum empréstimo')
                return render_template('AdmEmprestimos.html')
            
            return render_template('AdmEmprestimos.html', emprestimos=emprestimos)

    except Exception as e:
        flash('Erro ao buscar empréstimos')
        return redirect(url_for('perfilAdm')) 
...
def gerar_relatorio_livros():
    ...
    table = Table(data, colWidths=[110, 110, 90, 90, 90, 90])
    style = TableStyle([
...
def gerar_relatorio_emprestimos():
    ...
    table = Table(data, colWidths=[160, 160, 110, 110])
    style = TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
